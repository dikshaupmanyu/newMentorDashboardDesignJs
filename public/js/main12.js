...
const docRefreply = db.collection("/openGroups/roomOne/messages/"+id+"/replies/");
...
//  alert(PName)
 ...
 const docReply = db.collection("/openGroups/roomOne/messages/"+uniqueDocId+"/replies/"); 
 // alert(docReply);
...
function handleDeleteReply(obj1,obj2) {
  ...
  const docRefreply = db.collection("/openGroups/roomOne/messages/"+obj2+"/replies/");
  var txt;
        if (confirm("Are you sure you want to delete this chat ?")) {
           return docRefreply
          .doc(obj1)
...
//     querySnapshot.docChanges().forEach(function(change) {
   ...
   docRef.orderBy("createdDate", "desc").limit(100).onSnapshot(function(snapshot) {
      snapshot.docChanges().reverse().forEach(function(change) {
       ...
       const docRefreply = db.collection("/openGroups/roomOne/messages/"+change.doc.id+"/replies/");
         docRefreply.orderBy("createdDate", "asc").onSnapshot(function(snapshots) {       
